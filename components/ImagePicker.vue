<template>
  <ol v-if="data.hits" class="grid grid-cols-4 gap-2 overflow-y-auto overflow-x-auto h-64 p-2">
    <li
      ...
      class="h-12 relative overflow-hidden cursor-pointer outline outline-transparent focus:outline-blue-500 hover:outline-blue-500"
      :class="{ 'ring-2 ring-blue-500 shadow': modelValue === image.largeImageURL }"
      :key="image.id"
