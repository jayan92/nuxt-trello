<template>
  <div class="w-72 shadow dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 list flex flex-col">
    <!-- List Header  -->
    <div class="list-header p-2 border-b dark:border-gray-700 flex items-center justify-between cursor-grab">
      ...
      <div class="ml-auto">
        <UDropdown :items="listActions">
          <UButton size="sm" color="white" variant="link" icon="i-heroicons-cog-6-tooth"> </UButton>
        </UDropdown>
      </div>
    </div>
...
const { data, refresh } = await useFetch<CardDocument[]>(`/api/lists/${props.list._id}/cards`);
